const ReserveController = require('../models/ReserveModel');
const { readData, writeData } = require('../utils/Utilities');
const { getUpdatedBody } = require('../utils/ReserveUtility');
...
exports.createReservation = (req, res) => {
    ...
    let createReservation = new ReserveController(data.reservations.length + 1, contactName, phoneNumber, holidayObject);
...
exports.findReservationByHolidayId = (req, res) => {
    let holidayId = req.query;
    let data        = readData();
    let reservation = data.reservations.find(x => x.holiday.id === holidayId);

    reservation ? res.json(reservation) : res.status(404).send(`Reservation with this holiday id: ${holidayId} not found`);
}
